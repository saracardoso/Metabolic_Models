...
Metabolic_Models_Repo_model_reconstructions_T_CELLS_Treg = paste(Metabolic_Models_Repo_model_reconstructions_T_CELLS, 'subtypes_pipelines', 'Treg', sep='/')
Metabolic_Models_Repo_model_reconstructions_T_CELLS_Treg_PA_calls = paste(Metabolic_Models_Repo_model_reconstructions_T_CELLS_Treg, 'PA_calls', sep='/')
...
Treg_expression_data = read_omics_data(HumanGEM_genes,
                                       transcriptomics_data_files = list(RNAseq=Treg_rnaseq_data_file, Microarray=Treg_microarray_data_file),
...
Treg_PA_calls = PA_reaction_calls(Treg_expression_data, HumanGEM_GPR_file, HumanGEM_genes)

# Store data in files:
for(data_type in names(Treg_PA_calls$calls_per_sample$Transcriptomics)){
  file_name = paste(Metabolic_Models_Repo_model_reconstructions_T_CELLS_Treg_PA_calls, '/Treg_t_', data_type, '.csv', sep='')
  write.csv(Treg_PA_calls$calls_per_sample$Transcriptomics[[data_type]], file_name, row.names=T, col.names=T)
}
for(data_type in names(Treg_PA_calls$calls_per_sample$Proteomics)){
  file_name = paste(Metabolic_Models_Repo_model_reconstructions_T_CELLS_Treg_PA_calls, '/Treg_p_', data_type, '.csv', sep='')
  write.csv(Treg_PA_calls$calls_per_sample$Proteomics[[data_type]], file_name, row.names=T, col.names=T)
}
file_name = paste(Metabolic_Models_Repo_model_reconstructions_T_CELLS_Treg_PA_calls, 'Treg_gene_calls.csv', sep='/')
write.csv(Treg_PA_calls$gene_scores, file_name, row.names=T, col.names=F)
file_name = paste(Metabolic_Models_Repo_model_reconstructions_T_CELLS_Treg_PA_calls, 'Treg_reaction_calls.csv', sep='/')
write.csv(Treg_PA_calls$reaction_calls, file_name, row.names=T, col.names=T)
