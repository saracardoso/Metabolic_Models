...
Metabolic_Models_Repo_model_reconstructions_T_CELLS_CD8_memory = paste(Metabolic_Models_Repo_model_reconstructions_T_CELLS, 'subtypes_pipelines', 'CD8_memory', sep='/')
Metabolic_Models_Repo_model_reconstructions_T_CELLS_CD8_memory_PA_calls = paste(Metabolic_Models_Repo_model_reconstructions_T_CELLS_CD8_memory, 'PA_calls', sep='/')
...
CD8_memory_expression_data = read_omics_data(HumanGEM_genes,
...
CD8_memory_PA_calls = PA_reaction_calls(CD8_memory_expression_data, HumanGEM_GPR_file, HumanGEM_genes)

# Store data in files:
for(data_type in names(CD8_memory_PA_calls$calls_per_sample$Transcriptomics)){
  file_name = paste(Metabolic_Models_Repo_model_reconstructions_T_CELLS_CD8_memory_PA_calls, '/CD8_memory_t_', data_type, '.csv', sep='')
...
for(data_type in names(CD8_memory_PA_calls$calls_per_sample$Proteomics)){
  file_name = paste(Metabolic_Models_Repo_model_reconstructions_T_CELLS_CD8_memory_PA_calls, '/CD8_memory_p_', data_type, '.csv', sep='')
  write.csv(CD8_memory_PA_calls$calls_per_sample$Proteomics[[data_type]], file_name, row.names=T, col.names=T)
}
file_name = paste(Metabolic_Models_Repo_model_reconstructions_T_CELLS_CD8_memory_PA_calls, 'CD8_memory_gene_calls.csv', sep='/')
write.csv(CD8_memory_PA_calls$gene_scores, file_name, row.names=T, col.names=F)
file_name = paste(Metabolic_Models_Repo_model_reconstructions_T_CELLS_CD8_memory_PA_calls, 'CD8_memory_reaction_calls.csv', sep='/')
