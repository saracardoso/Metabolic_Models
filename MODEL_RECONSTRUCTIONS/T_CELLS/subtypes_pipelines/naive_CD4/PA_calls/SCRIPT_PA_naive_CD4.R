...
Metabolic_Models_Repo_model_reconstructions_T_CELLS_naive_CD4 = paste(Metabolic_Models_Repo_model_reconstructions_T_CELLS, 'subtypes_pipelines', 'naive_CD4', sep='/')
Metabolic_Models_Repo_model_reconstructions_T_CELLS_naive_CD4_PA_calls = paste(Metabolic_Models_Repo_model_reconstructions_T_CELLS_naive_CD4, 'PA_calls', sep='/')
...
naive_CD4_expression_data = read_omics_data(HumanGEM_genes,
...
naive_CD4_PA_calls = PA_reaction_calls(naive_CD4_expression_data, HumanGEM_GPR_file, HumanGEM_genes)

# Store data in files:
for(data_type in names(naive_CD4_PA_calls$calls_per_sample$Transcriptomics)){
 file_name = paste(Metabolic_Models_Repo_model_reconstructions_T_CELLS_naive_CD4_PA_calls, '/naive_CD4_t_', data_type, '.csv', sep='')
 write.csv(naive_CD4_PA_calls$calls_per_sample$Transcriptomics[[data_type]], file_name, row.names=T, col.names=T)
}
for(data_type in names(naive_CD4_PA_calls$calls_per_sample$Proteomics)){
  file_name = paste(Metabolic_Models_Repo_model_reconstructions_T_CELLS_naive_CD4_PA_calls, '/naive_CD4_p_', data_type, '.csv', sep='')
  write.csv(naive_CD4_PA_calls$calls_per_sample$Proteomics[[data_type]], file_name, row.names=T, col.names=T)
}
file_name = paste(Metabolic_Models_Repo_model_reconstructions_T_CELLS_naive_CD4_PA_calls, 'naive_CD4_gene_calls.csv', sep='/')
write.csv2(naive_CD4_PA_calls$gene_scores, file_name, row.names=T, col.names=F)
file_name = paste(Metabolic_Models_Repo_model_reconstructions_T_CELLS_naive_CD4_PA_calls, 'naive_CD4_reaction_calls.csv', sep='/')
write.csv(naive_CD4_PA_calls$reaction_calls, file_name, row.names=T, col.names=T)
