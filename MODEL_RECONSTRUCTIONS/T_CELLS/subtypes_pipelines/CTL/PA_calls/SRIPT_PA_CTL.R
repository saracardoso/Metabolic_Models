...
Metabolic_Models_Repo_model_reconstructions_T_CELLS_CTL = paste(Metabolic_Models_Repo_model_reconstructions_T_CELLS, 'subtypes_pipelines', 'CTL', sep='/')
Metabolic_Models_Repo_model_reconstructions_T_CELLS_CTL_PA_calls = paste(Metabolic_Models_Repo_model_reconstructions_T_CELLS_CTL, 'PA_calls', sep='/')
...
T_Cell_Data_Repo_Data_Files_CTL_RNA_seq = paste(T_Cell_Data_Repo_Data_Files_CTL, 'RNA_seq', sep='/')
...
CTL_rnaseq_data_file = paste(T_Cell_Data_Repo_Data_Files_CTL_RNA_seq, 'CTL_rnaseq_data.csv', sep='/')
CTL_proteomics_data_file = paste(T_Cell_Data_Repo_Data_Files_CTL_Proteomics, 'CTL_proteomics_data.csv', sep='/')
CTL_microarray_metadata_file = paste(T_Cell_Data_Repo_Data_Files_CTL_Microarray, 'CTL_microarray_metadata.csv', sep='/')
CTL_rnaseq_metadata_file = paste(T_Cell_Data_Repo_Data_Files_CTL_RNA_seq, 'CTL_rnaseq_metadata.csv', sep='/')
...
CTL_expression_data = read_omics_data(HumanGEM_genes,
...
CTL_PA_calls = PA_reaction_calls(CTL_expression_data, HumanGEM_GPR_file, HumanGEM_genes)

# Store data in files:
for(data_type in names(CTL_PA_calls$calls_per_sample$Transcriptomics)){
  file_name = paste(Metabolic_Models_Repo_model_reconstructions_T_CELLS_CTL_PA_calls, '/CTL_t_', data_type, '.csv', sep='')
  write.csv(CTL_PA_calls$calls_per_sample$Transcriptomics[[data_type]], file_name, row.names=T, col.names=T)
}
file_name = paste(Metabolic_Models_Repo_model_reconstructions_T_CELLS_CTL_PA_calls, 'CTL_gene_calls.csv', sep='/')
write.csv(CTL_PA_calls$gene_scores, file_name, row.names=T, col.names=F)
file_name = paste(Metabolic_Models_Repo_model_reconstructions_T_CELLS_CTL_PA_calls, 'CTL_reaction_calls.csv', sep='/')
