...
### 
plot_df = c()
for(rxn in c('MAR05029', 'MAR01378', 'MAR05450', 'MAR06895', 'MAR08884')){
  if(length(plot_df)==0) plot_df = cbind(as.numeric(fba_fluxes[rxn,]),
                                         metadata, rep(rxn, dim(fba_fluxes)[2]))
  else plot_df = rbind(plot_df, cbind(as.numeric(fba_fluxes[rxn,]),
                                      metadata, rep(rxn, dim(fba_fluxes)[2])))
}
colnames(plot_df)[c(1,7)] = c('flux', 'reaction')

ggplot2::ggplot(plot_df, ggplot2::aes(x=cell_type, y=flux, colour=cell_type)) +
  ggplot2::geom_boxplot() + ggplot2::scale_colour_manual(values = ct_colors) +
  ggplot2::theme(axis.text.x=ggplot2::element_text(angle=30, hjust=1, vjust=1))
ggplot2::ggplot(plot_df, ggplot2::aes(x=cell_type, y=flux, colour=simple_cts)) +
  ggplot2::geom_boxplot() + ggplot2::scale_colour_manual(values = simple_ct_colors) +
  ggplot2::theme(axis.text.x=ggplot2::element_text(angle=30, hjust=1, vjust=1))


ggplot2::ggplot(plot_df[plot_df$cell_type=='Proliferative CD4 Tcells',], ggplot2::aes(x=reaction, y=flux, colour=state)) +
  ggplot2::geom_point() + ggplot2::scale_colour_manual(values = state_colors) +
  ggplot2::facet_wrap(ggplot2::vars(individual)) +
  ggplot2::theme(axis.text.x=ggplot2::element_text(angle=30, hjust=1, vjust=1))


plot_df2 = c()
for(rxn in important_rxns$uptakes$glutamine){
  if(length(plot_df2)==0) plot_df2 = cbind(as.numeric(fba_fluxes[rxn,]),
                                         metadata, rep(rxn, dim(fba_fluxes)[2]))
  else plot_df2 = rbind(plot_df2, cbind(as.numeric(fba_fluxes[rxn,]),
                                      metadata, rep(rxn, dim(fba_fluxes)[2])))
}
colnames(plot_df2)[c(1,7)] = c('flux', 'reaction')
ggplot2::ggplot(plot_df2[plot_df2$cell_type=='Cytotoxic CD8 Tcells',], ggplot2::aes(x=reaction, y=flux, colour=state)) +
  ggplot2::geom_point() + ggplot2::scale_colour_manual(values = state_colors) +
  ggplot2::facet_wrap(ggplot2::vars(individual)) +
  ggplot2::theme(axis.text.x=ggplot2::element_text(angle=30, hjust=1, vjust=1))
