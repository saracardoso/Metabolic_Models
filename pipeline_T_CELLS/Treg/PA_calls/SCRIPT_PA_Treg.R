...
Treg_expression_data = read_omics_data(recon3D_consistent_genes, recon3DModel_gene_mapping_file,
...
Treg_PA_calls = PA_reaction_calls(Treg_expression_data, recon3DModel_gene_mapping_file, recon3D_consistent_GPR_file, recon3D_consistent_genes)

# Store data in files:
for(data_type in names(Treg_PA_calls$calls_per_sample$Transcriptomics)){
  file_name = paste(Metabolic_Models_Repo_reconstruction_process_T_CELLS_Treg_PA_calls, '/Treg_t_', data_type, '.csv', sep='')
...
for(data_type in names(Treg_PA_calls$calls_per_sample$Proteomics)){
  file_name = paste(Metabolic_Models_Repo_reconstruction_process_T_CELLS_Treg_PA_calls, '/Treg_p_', data_type, '.csv', sep='')
  write.csv(Treg_PA_calls$calls_per_sample$Proteomics[[data_type]], file_name, row.names=T, col.names=T)
}
file_name = paste(Metabolic_Models_Repo_reconstruction_process_T_CELLS_Treg_PA_calls, 'Treg_gene_calls.csv', sep='/')
write.csv(Treg_PA_calls$gene_scores, file_name, row.names=T, col.names=F)
file_name = paste(Metabolic_Models_Repo_reconstruction_process_T_CELLS_Treg_PA_calls, 'Treg_reaction_calls.csv', sep='/')
